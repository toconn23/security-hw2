# DES Decryption from Scratch (No Libraries)

# Define all necessary tables (Initial Permutation, Expansion, S-boxes, etc.)
IP = [
...
def permute(block, table):
    #permute the block with the permutation table
    res = [] 
    for i in table:
...
def generate_subkeys(key):
    print(f"******KEY GEN********")
    print(f"Original key: {convert_to_string(key)}")
    #apply PC1 permutation to the key
    key = permute(key, PC1)
    print(f"After PC1: {convert_to_string(key)}")
    print("")
    ...
    for i in range(16):
        ...
        #rotate with shift schedule
        left = left_rotate(left, SHIFT_SCHEDULE[i])
        right = left_rotate(right, SHIFT_SCHEDULE[i])
        
        combined = left + right 
        #apply PC2 permutation to get the subkey
        subkey = permute(combined, PC2)
        subkeys.append(subkey)
        print(f"Key {i+1}: {convert_to_string(subkey)}")
        print("")
...
def s_box_substitution(expanded_block):
    output = []
    #split block into 8 6-bit blocks
    for i in range(8):
...
def f_function(R, subkey):
    ...
    #apply sbox sub and permute
    substituted = s_box_substitution(xored)
    permuted = permute(substituted, P)
    print(f"Result of f_function: {convert_to_string(permuted)}")
    return permuted
...
def des_decrypt(ciphertext, key):
    ...
    #apply 16 rounds, using subkeys in reverse order for decryption
    for i in range(15, -1, -1):
        print(f"For key {i+1}:")
        next_L = R
        next_R = xor(L, f_function(R, subkeys[i]))
        L = next_L
        R = next_R
        print(f"L{16-i}: {convert_to_string(L)}")
        print(f"R{16-i}: {convert_to_string(R)}")
        print("")
    
    #swap L and R for the final permutation
    combined = R + L
    print(f"after all round combine result: {convert_to_string(combined)}")
    
    #apply and return final permutation
    plaintext = permute(combined, IP_inverse)
    print(f"Result of final permutation: {convert_to_string(plaintext)}")
    return plaintext

#convert string representations to list of bits
